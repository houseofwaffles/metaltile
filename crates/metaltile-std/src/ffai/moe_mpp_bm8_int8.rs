...
/// New-syntax correctness test for the MPP MoE int8 BGEMM (BM=8). Oracle is the
/// shared per-row-`indices` int8 dequant-then-grouped-matmul (4 unsigned bytes
/// per u32). Inputs are dtype-rounded; tolerance is wide because the MPP
/// cooperative-tensor accumulator reorders the K reduction.
///
...
/// New-syntax benchmark for the MPP MoE int8 BGEMM (BM=8). `bits=8` →
...
pub mod kernel_benches {
    ...
    #[bench(name = "ffai/moe_mpp/gather_qmm_mma_int8_bm8", dtypes = [f32, f16, bf16])]
