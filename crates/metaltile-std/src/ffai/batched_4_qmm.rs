...
//! `x: [M, in_dim]` and produce row `m` of FOUR separate output
//! tensors:
//!   a_buf: [M, out_a] T
//!   b_buf: [M, out_b] T
//!   c_buf: [M, out_c] T
//!   d_buf: [M, out_d] T
//!
//! Four buffers (rather than one concatenated output) keep each
//! projection contiguous in memory so the downstream consumer reads
//! `[M, dim]` per projection without strided / column-slice tensor
//! views. Callers can still alias all four into one backing allocation
//! if they want; the kernel only sees four base pointers.
//!
//! Dispatch geometry mirrors `ffai_batched_4_qgemv_fast`:
//!
//!   * `program_id::<2>()` selects matrix (0 = A, 1 = B, 2 = C, 3 = D).
//!   * `program_id::<1>()` selects the batched row m (0..M).
//!   * `program_id::<0>()` selects an 8-row output tile (TG covers
//!     8 output rows of the chosen matrix). TPG = 64 = 2 SG × 32 lanes.
//!
//! Grid: `[ceil(max(out_a, out_b, out_c, out_d) / 8), M, 4]`,
//! TPG = `[64, 1, 1]`.
//!
//! The inner loop is the algebraic-split `s*q_dot + b*xs` trick from
//! `ffai_batched_qkv_qmm_fast` / `ffai_batched_4_qgemv_fast` — same
//! mask-without-shift constants, same per-512-chunk x preload.
//!
//! Constraints (same as the GEMV-fast 4-output sibling):
//!
//!   * `in_dim % 512 == 0`
//!   * `out_a`, `out_b`, `out_c`, `out_d` each multiple of 8
//!   * `group_size == 64`
//!
//! Note: GDN projections like `b` / `a` are very small (`num_value_heads`
//! ≈ 16). The kernel handles unequal out dims via the per-matrix
//! `row0 < out_*` guard — TGs past a matrix's `out_*` rows no-op cleanly.
//!
//! Quantized-weight layout (N = `in_dim`, G = `group_size`, int4) —
//! identical to the GEMV variant; only `x` and `out` change shape:
//!   x         [M, N]        T
//!   w_*       [out_*, N/8]  uint32
//!   scales_*  [out_*, N/G]  T
//!   biases_*  [out_*, N/G]  T
...
//! `tests/batched_4_qmm_gpu_correctness.rs` (oracle = running the
//! 4-output GEMV-fast kernel M times and concatenating row outputs).
    ...
    if matrix == 0u32 {
        if row0 < out_a {
            for _b in range(0u32, in_dim, 512u32) {
                ...
                // Group index uses the in-row x offset (not the batched
                // global x offset) since scales/biases are per weight
                // row × group, not per batched row.
            ...
            if lane == 0u32 {
                store(a_buf[a_row_off + row0], r0.cast::<T>());
                store(a_buf[a_row_off + row1], r1.cast::<T>());
                store(a_buf[a_row_off + row2], r2.cast::<T>());
                store(a_buf[a_row_off + row3], r3.cast::<T>());
            }
        }
    }
    if matrix == 1u32 {
        if row0 < out_b {
            ...
            let r3 = simd_sum(acc3);
            if lane == 0u32 {
                store(b_buf[b_row_off + row0], r0.cast::<T>());
                store(b_buf[b_row_off + row1], r1.cast::<T>());
                store(b_buf[b_row_off + row2], r2.cast::<T>());
                store(b_buf[b_row_off + row3], r3.cast::<T>());
            }
        }
    }
    if matrix == 2u32 {
        if row0 < out_c {
            ...
            let r3 = simd_sum(acc3);
            if lane == 0u32 {
                store(c_buf[c_row_off + row0], r0.cast::<T>());
                store(c_buf[c_row_off + row1], r1.cast::<T>());
                store(c_buf[c_row_off + row2], r2.cast::<T>());
                store(c_buf[c_row_off + row3], r3.cast::<T>());
            }
        }
    }
    if matrix == 3u32 {
        if row0 < out_d {
            ...
            let r3 = simd_sum(acc3);
            if lane == 0u32 {
                store(d_buf[d_row_off + row0], r0.cast::<T>());
                store(d_buf[d_row_off + row1], r1.cast::<T>());
                store(d_buf[d_row_off + row2], r2.cast::<T>());
                store(d_buf[d_row_off + row3], r3.cast::<T>());
            }
        }
    }
}
