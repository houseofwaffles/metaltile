...
//! Single-token SDPA decode for `head_dim == 96`. Parallel of
//! `ffai_sdpa_decode` (head_dim=128) with 3 elements per lane instead
//! of 4 — see that file for the full algorithm walkthrough.
//!
//! Needed for production models in the head_dim=96 bracket:
//!   - Phi-3-mini / Phi-3.5-mini (3072 hidden / 32 heads = 96 head_dim)
//!   - Other 96-wide research / instruct variants
//!
//! Single-phase output reduction (3 × 1056 floats ≈ 12 KB threadgroup
//! memory, well under Apple's 32 KB cap), like the head_dim=64 kernel —
//! no need for the head_dim=256 kernel's two-phase split.
//!
//! ## DISPATCH INVARIANTS
//!
//! Same shape as `ffai_sdpa_decode`, with the per-lane width at 3:
//!
//! - **TPG = 1024 threads** (32 simdgroups × 32 lanes). Each lane
//!   owns 3 consecutive Q/K/V elements (`head_dim / 32 = 96 / 32 = 3`),
//!   loaded unconditionally at `lane * 3 + {0, 1, 2}`.
//! - **`head_dim == 96`.** Wrapper-enforced; other head dims belong
//!   to `ffai_sdpa_decode` (128), `ffai_sdpa_decode_d64` (64),
//!   `ffai_sdpa_decode_d256` (256), `ffai_sdpa_decode_d512` (512).
...
//!   capacity.
//!
//! Dense full attention only (no sink / window params) — the same as
//! the d64 / d256 / d512 kernels. Wrapping doc: see
//! FFAI/CLAUDE.md §"Wrapping kernels in FFAI".
...
pub fn ffai_sdpa_decode_d96<T>(
    ...
    let lane = simd_lane;
    let ns = n_simd;
    // Three tg_out slots — each lane owns 3 output elements at
    // head_dim=96. Same `+32` bank-conflict padding rationale as the
    // head_dim=64 / 128 kernels.
    ...
    let d0 = lane * 3u32;
    // Pre-scale this lane's 3-element Q stripe once; K/V are streamed.
...
mod tests {
    ...
    use super::ffai_sdpa_decode_d96;
    use crate::bench_types::DType;

    fn msl_for(dt: DType) -> String {
        let mut k = ffai_sdpa_decode_d96::kernel_ir_for(dt);
        k.mode = KernelMode::Reduction;
        MslGenerator::default().generate(&k).expect("ffai_sdpa_decode_d96 codegen succeeds")
    }
    ...
    fn codegen_produces_nonempty_msl_for_all_float_dtypes() {
        for dt in [DType::F32, DType::F16, DType::BF16] {
            ...
            assert!(
                src.contains("kernel void ffai_sdpa_decode_d96"),
                "MSL for {dt:?} should declare ffai_sdpa_decode_d96:\n{src}",
            );
        }
    }
}

/// New-syntax correctness for `ffai_sdpa_decode_d96` (head_dim 96, 3 elems per
/// lane; Reduction, one threadgroup per q-head). Oracle is the standard
/// softmax(QKᵀ·scale)·V decode with GQA via `heads_per_group`.
pub mod kernel_tests {
    ...
    fn test_ffai_sdpa_decode_d96(dt: DType) -> TestSetup {
        let (n_q_heads, n_kv_heads, head_dim) = (8usize, 4usize, 96usize);
...
/// New-syntax benchmark for `ffai_sdpa_decode_d96` (`class=GenericEmpty`).
pub mod kernel_benches {
    ...
    fn bench_sdpa_decode_d96(dt: DType) -> BenchSetup {
        let (n_q_heads, n_kv_heads, head_dim) = (32usize, 8usize, 96usize);
