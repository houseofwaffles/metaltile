...
//!
//! `quantize_kv_body!($bits)` and `bulk_dequant_kv_body!($bits)` share
//! identical logic across int4 and int8; `$bits` is baked in as a
//! literal so Metal constant-folds all pack-size arithmetic at PSO
//! creation.
//!
...
// ─── Affine quantize — shared body ───────────────────────────────────
//
// One thread per group.  Scans the group for min/max, then packs
// `vals_per_pack = 32/bits` quantized values per u32 word.
