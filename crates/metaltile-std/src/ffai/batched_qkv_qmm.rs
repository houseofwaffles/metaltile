...
//! `ffai_rms_norm_qgemv_fast` / `ffai_batched_qkv_qgemv_fast` — same
//! mask-without-shift constants, same per-512-chunk x preload — minus the
//! RMSNorm phase.
//!
//! Constraints (same as the fast GEMV variant):
//!
//!   * `in_dim % 512 == 0`
//!   * `out_q`, `out_k`, `out_v` each multiple of 8
//!   * `group_size == 64`
//!
...
//!   scales_*  [out_*, N/G]            T
//!   biases_*  [out_*, N/G]            T
//!   out       [M, out_q+out_k+out_v]  T
//!
//! Codegen-only; correctness pinned by
//! `tests/batched_qkv_qmm_gpu_correctness.rs` (oracle = running the
//! GEMV-fast kernel M times and concatenating row outputs).
