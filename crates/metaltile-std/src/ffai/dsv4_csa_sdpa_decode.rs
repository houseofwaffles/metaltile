...
//! same. Per the playbook §"When NOT to dedup" → "mask-on/mask-off
//! variants of an SDPA kernel — sometimes a shared body with a
//! conditional is cleanest, sometimes parallel kernels", keeping
//! them parallel preserves the cleanest emitted MSL for each path
//! (no `if gather` per inner-loop iter).
