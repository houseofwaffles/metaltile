...
pub mod kernel_tests {
    ...
    /// Dense softmax-attention oracle. The online-softmax flash decode
    /// produces the same result as a numerically-stable dense
    /// `softmax(QKᵀ·scale)·V` over the K,V cache (with GQA fan-out).
