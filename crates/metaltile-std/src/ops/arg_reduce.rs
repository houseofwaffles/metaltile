...
// ─── Generic argmax<T> with u32 index output (FFAI-specific) ─────────
//
// Adapted from `mt_argmax_f32` above but generic over input dtype and
// emitting a `u32` index (not a float-cast version). FFAI's GPU sampler
// needs an integer token id; the f32-output upstream variant doesn't
// fit that contract.
//
